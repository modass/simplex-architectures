...
Point Simulator<Automaton>::getBaseControllerOutput() {
...
template <typename Automaton>
hypro::TRIBOOL Simulator<Automaton>::isSafe( const Point& ctrlInput ) {
  roots.clear();
...
template <typename Automaton>
void Simulator<Automaton>::update( const Point& ctrlInput, const Point& nextObservation ) {
    ...
    for ( auto& n : hypro::preorder( r ) ) {
      if ( n.isLeaf() ) {
        ...
        if ( containment != hypro::CONTAINMENT::FULL ) {
          ...
          spdlog::warn(
              "Leaf node ({}, parent {}, parent initial: {}) initial set {} should be fully contained in tick = 0, but "
              "is actually not.",
          ...
          spdlog::warn( "Node flags: timelock: {}, bad state: {}, has fixed point: {}, is on Zeno-cycle: {}",
                        ...
                        n.isOnZenoCycle() );
          throw std::logic_error( "Leaf node initial set " + ss.str() +
                                  " should be fully contained in tick = 0, but is actually not." );
        }
...
template <typename Automaton>
void Simulator<Automaton>::setCtrlValue( Point& sample, const Point& ctrlInput ) {
  // augment state with controller input
  sample.at( 2 ) = ctrlInput.at( 0 );
}

template <typename Automaton>
std::map<LocPtr, std::vector<Box>> Simulator<Automaton>::potentialNextStates() {
  for ( auto& root : roots ) {
    cutoffControllerJumps( &root );
  }
