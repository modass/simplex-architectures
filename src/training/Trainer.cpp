...
void Trainer::plot( const std::string& outfilename ) {
  ...
  for ( const auto& [locationName, tree] : mTrees ) {
    spdlog::debug("Plot tree for location {} which stores {} sets to file {}", locationName, tree.size(), outfilename+"_"+locationName);
...
void Trainer::runIteration( const hypro::Settings& settings, InitialStatesGenerator& generator ) {
  ...
  auto initialBox = Representation(newInitialStates.begin()->second.getMatrix(), newInitialStates.begin()->second.getVector());
  std::stringstream ss;
  ss << initialBox;
  if(mTrees.at(newInitialStates.begin()->first->getName()).contains(initialBox)) {
    spdlog::debug("Initial set {} for location {} already contained in octrees, skip iteration.", ss.str(), newInitialStates.begin()->first->getName());
    return;
  }
