...
hypro::Condition<Number> GeneralRoadSegment::getLeftZoneHeadingGuard( double relativeCenterZoneWidth, double thetaMin, double thetaMax ) {
  auto pointA = startLeft;
  auto pointB = endLeft;
  auto pointC = getCenterEndLeft(relativeCenterZoneWidth);
  auto pointD = getCenterStartLeft(relativeCenterZoneWidth);

  return pointsAndHeadingToCondition(pointA, pointB, pointC, pointD, thetaMin, thetaMax);
}
hypro::Condition<Number> GeneralRoadSegment::getCenterZoneHeadingGuard( double relativeCenterZoneWidth, double thetaMin, double thetaMax  ) {
  auto pointA = getCenterStartLeft(relativeCenterZoneWidth);
  auto pointB = getCenterEndLeft(relativeCenterZoneWidth);
  auto pointC = getCenterEndRight(relativeCenterZoneWidth);
  auto pointD = getCenterStartRight(relativeCenterZoneWidth);
...
hypro::Condition<Number> GeneralRoadSegment::getRightZoneHeadingGuard( double relativeCenterZoneWidth, double thetaMin, double thetaMax  ) {
  auto pointA = getCenterStartRight(relativeCenterZoneWidth);
  auto pointB = getCenterEndRight(relativeCenterZoneWidth);
  auto pointC = endRight;
  auto pointD = startRight;
...
hypro::Condition<Number> GeneralRoadSegment::pointsAndHeadingToCondition(Point pointA, Point pointB, Point pointC, Point pointD, double thetaMin, double thetaMax) {
  using Matrix = hypro::matrix_t<double>;
  using Vector = hypro::vector_t<double>;

  Matrix constraints  = Matrix::Zero( 6, numVariables );
  ...
  constraints( 4, theta ) = 1;
  constants(4) = thetaMax;
  constraints( 5, theta ) = -1;
  constants(5) = -thetaMin;
